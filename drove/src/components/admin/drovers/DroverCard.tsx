  ...
  if (tipo === "core") {
    return (
      <span
        ...
        style={{ color: "#6EF7FF", fontFamily: "Helvetica", letterSpacing: 0.5 }}
        title="Contratado por Drove"
      >
        {/* Puedes cambiar el icono si quieres */}
        <span className="text-white font-helvetica" style={{ textShadow: "0 1px 2px #0008" }}>DROVER</span>
        <span style={{ color: "#6EF7FF", fontWeight: 700, marginLeft: 2 }}>Core</span>
      </span>
    );
  }
  return (
    ...
    >
      <span className="text-white font-helvetica" style={{ textShadow: "0 1px 2px #0008" }}>DROVER</span>
      <span style={{ color: "#52ff99", fontWeight: 700, marginLeft: 2 }}>Flex</span>
    </span>
  );
};

const DroverCard: React.FC<DroverCardProps> = ({ drover, onClickPerfil }) => (
  <div className="bg-[#1A1F2C] rounded-2xl shadow-lg p-5 flex flex-col items-center transition-all hover:shadow-[0_0_30px_1px_#6EF7FF55] hover:-translate-y-1 cursor-pointer duration-150">
    <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-3 text-3xl font-bold uppercase ${
      drover.avatar 
        ? "" 
        : "bg-[#6ef7ff]/70 text-[#22142A]"
    } overflow-hidden`}>
      {drover.avatar
        ? <img src={drover.avatar} alt={drover.nombre} className="w-full h-full rounded-full object-cover"/>
    ...
    <div className="mt-1"><span className="text-white/70 text-xs">{drover.email}</span></div>
    ...
    <div className="flex flex-col items-center gap-1 text-white/60 text-xs mt-2 mb-2">
      ...
      <div>⭐ {drover.calificacion} | <span>Promedio {drover.tiempo}</span></div>
    </div>
    <button 
