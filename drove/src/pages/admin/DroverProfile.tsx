...
const tipoBadge = tipo => (
  <span className={`flex rounded-full px-2.5 py-1 text-xs font-semibold ml-1 ${tipo === "drover_core"
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between px-3 xs:px-4 md:px-0">
        <Button variant="outline" onClick={() => navigate(-1)} className="rounded-xl border-2 border-white/20 bg-transparent text-[#6EF7FF] hover:bg-[#6EF7FF] hover:text-[#22142A] h-10 px-6 py-2">
          <ArrowLeft size={16} className="mr-2" /> Volver a Drovers
        </Button>
        <div className="flex items-center gap-2">
          {drover?.role !== "drover_core" && (
            <Button onClick={handleHire} variant="outline" className="rounded-xl bg-green-700 text-white hover:text-green-900 hover:bg-green-200 font-bold h-10 px-6 py-2">
