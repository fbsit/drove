  ...
  const handleGetUser = async () => {
    try {
      ...
      setClient({
        ...
        vehiculosTrasladados: summary?.vehicleStats?.map((v: any) => ({ tipo: v.type, cantidad: v.count })) || [],
        rutasFavoritas: summary?.favoriteRoutes?.map((r: any) => ({ origen: r.origin, destino: r.destination, veces: r.count })) || [],
        gamificacion: { traslados: summary?.tripsCount || 0 },
        gastoTotal: summary?.totalSpent || 0,
        contactInfo: summary?.contactInfo || {},
        tipo: (summary?.contactInfo?.companyName ? 'empresa' : 'persona') as any,
      });
    } catch (e) {
    ...
    return (
      <div className="p-8 text-white">
        <div className="flex items-center justify-center h-40">
          <svg className="animate-spin mr-3 h-6 w-6 text-[#6EF7FF]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div className="bg-transparent p-0 animate-fade-in">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-4 lg:gap-6">
            ...
            <div className="flex-1 min-w-0 text-center lg:text-left bg-yellow-900">
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 items-center justify-center lg:justify-start mb-3">
                <h1 className="text-xl sm:text-2xl lg:text-3xl text-white font-bold break-words" style={{ fontFamily: "Helvetica" }}>
                  {client?.contactInfo?.fullName}
                </h1>
                <div className="flex flex-wrap gap-2 items-center justify-center lg:justify-start">
                  {tipoBadge(client.tipo)}
                  <span className="px-3 py-1 rounded-full font-semibold text-sm bg-amber-500 text-black">
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-center justify-center lg:justify-start text-white/80 text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-5">
        ...
        <div className="rounded-2xl bg-white/5 border border-white/15 p-5 text-center">
          ...
          <div className="text-4xl font-bold text-[#6EF7FF]">{gastoTotal.toLocaleString("es-ES", { style: "currency", currency: "EUR" })}</div>
        </div>
        <div className="rounded-2xl bg-white/5 border border-white/15 p-5 text-center">
          <span className="text-white/70 text-sm block mb-1">Promedio</span>
          <div className="text-4xl font-bold text-[#6EF7FF]">{gastoPromedio.toLocaleString("es-ES", { style: "currency", currency: "EUR" })}</div>
        </div>
      </div>
